...
class ExpiredReservationsCronJob(CronJobBase):
    ...
    The `ExpiredReservationsCronJob` class is a scheduled task that updates the
    status of reservations when their date has 'expired'.

    - The cron job is configured to run every 1440 minutes (24 hours) using
        the `django_cron` library.
    - It filters reservations with a date earlier than today and a status
        of 'active' (0).
    ...
    def do(self):
        ...
        expired_reservations = Reservation.objects.filter(
            date__lt=date.today(), reservation_status=0
        )
